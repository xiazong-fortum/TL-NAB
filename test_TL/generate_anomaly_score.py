...
def generate_anomaly_score(inputData, model_file, scaler, sequence_length, batch_size=64):
    ...
    for column in oil_pressure_columns:
        inputData.loc[:, column] = scaler.transform(inputData.loc[:, column].values.reshape(-1,1))
...
def process_and_save_anomaly_scores(data_files, model_files, root_dir, scaler, sequence_length):
    for model_file in model_files:
        for file in data_files:
            ...
            anomaly_scores = generate_anomaly_score(df, os.path.join(root_dir, model_file), scaler, sequence_length)
            ...
            output_file = os.path.join(root_dir, file.replace('.csv', f'_{model_name}_scored.csv'))
